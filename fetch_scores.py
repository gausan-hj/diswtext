...
function getTomorrowUniform() {{
    ...
    const weekday = tomorrow.toLocaleDateString(currentLang === 'zh' ? 'zh-CN' : 'en-US', {{ weekday: 'long' }});
    
    const tomorrowCCA = CCA_DATA.find(item => item.date === dateStr);
    
    if (tomorrowCCA) {{
        const activity = currentLang === 'en' ? translateActivity(tomorrowCCA.activity) : tomorrowCCA.activity;
        if (tomorrowCCA.uniform === "体育衣") {{
            return currentLang === 'en' 
                ? `🎽 Tomorrow (${{weekday}}) wear sports uniform! Activity: ${{activity}}`
                : `🎽 明天${{weekday}}请穿体育衣！活动：${{activity}}`;
        }} else {{
            return currentLang === 'en'
                ? `👔 Tomorrow (${{weekday}}) wear school uniform. Activity: ${{activity}}`
                : `👔 明天${{weekday}}穿校服，活动：${{activity}}`;
        }}
    }} else {{
        return currentLang === 'en'
            ? `👔 Tomorrow (${{weekday}}) wear school uniform, bring name tag`
            : `👔 明天${{weekday}}穿整齐校服，记得带名牌`;
    }}
}}
...
function translateActivity(activity) {{
    const activityMap = {{
        '活动顾问+总学长+副总学长': 'Activity: Advisor + Head Prefect + Deputy',
        '操步': 'Marching',
        '活动（文书+财政+查账）': 'Activity (Secretary + Treasurer + Auditor)',
        '活动（行动组）': 'Activity (Action Team)',
        '活动（值岗组）': 'Activity (Duty Team)',
        '活动（督察组）': 'Activity (Discipline Team)',
        '活动（星穹组）': 'Activity (Starry Group)',
        '活动（夜曜组）': 'Activity (Night Group)',
        '活动（沧澜组）': 'Activity (Ocean Group)',
        '第一学期检讨': 'First Semester Review'
    }};
