...
html += '''
    ...
    <div class="notification-container">
        <div class="notification-bell" id="notificationBell">
            ...
            <span class="bell-text">开启每日提醒</span>
        </div>
        <button class="test-notification-btn" id="testNotificationBtn">
            <span class="test-icon">📱</span>
            <span class="test-text">测试通知</span>
        </button>
    </div>
    ...
    <div class="time-picker" id="timePicker">
        ...
        <div class="time-option">
            <label>早上 6:00（起床提醒）</label>
            <input type="checkbox" value="6" class="time-checkbox">
        </div>
        <div class="time-option">
            <label>晚上 7:00（明天衣服）</label>
        ...
        <div class="time-option">
            ...
            <input type="checkbox" value="22" class="time-checkbox">
        </div>
        <button class="time-confirm" id="confirmTime">确认开启提醒</button>
    </div>
    ...
    <div class="notification-toast" id="notificationToast">
        ...
        <div class="toast-title">
            <span>🔔</span>
            <span id="toastMessage">通知已开启</span>
        </div>
        <div style="font-size: 0.8rem; color: var(--text-secondary);" id="toastDetail">
            每天将在您选择的时间提醒
        </div>
    </div>
...
html += '''        ];
        ...
        class NotificationManager {
            constructor() {
                ...
                this.testBtn = document.getElementById('testNotificationBtn');
                this.timePicker = document.getElementById('timePicker');
                this.toast = document.getElementById('notificationToast');
                this.toastMessage = document.getElementById('toastMessage');
                this.toastDetail = document.getElementById('toastDetail');
                this.checkboxes = document.querySelectorAll('.time-checkbox');
                this.confirmBtn = document.getElementById('confirmTime');
            ...
            init() {
                ...
                this.bell.addEventListener('click', () => {
                    if (this.checkPermission()) {
                        this.timePicker.classList.add('show');
                    } else {
                ...
                this.confirmBtn.addEventListener('click', () => {
                    this.saveSettings();
                    this.timePicker.classList.remove('show');
                    this.showToast('✅ 提醒已开启', '每天将在您选择的时间发送通知');
                });
                
                // 点击外部关闭时间段选择器
                document.addEventListener('click', (e) => {
                    if (!this.timePicker.contains(e.target) && !this.bell.contains(e.target)) {
                        this.timePicker.classList.remove('show');
                    }
                });
            }
            ...
            requestPermission() {
                Notification.requestPermission().then(perm => {
                    if (perm === 'granted') {
                        this.showToast('✅ 通知权限已开启', '现在可以设置提醒时间了');
                        this.timePicker.classList.add('show');
                    } else {
                        this.showToast('❌ 通知被拒绝', '您可以在浏览器设置中重新开启');
                    }
                });
            }
            ...
            testNotification() {
                ...
                if (Notification.permission === 'granted') {
                    // 已经允许，直接发送测试
                    const testNotif = new Notification('✅ 测试成功', {
                        body: '如果你看到这条通知，说明你的手机支持通知功能！\n（注意：手机锁屏后可能收不到）',
                    ...
                    this.showToast('📱 测试通知已发送', '请查看手机通知栏');
                } else if (Notification.permission === 'denied') {
                    this.showToast('❌ 通知被拒绝', '请在浏览器设置中允许通知');
                } else {
                    // 还没请求过权限
                    Notification.requestPermission().then(perm => {
                        if (perm === 'granted') {
                            this.testNotification(); // 递归调用，现在会发送
                        } else {
                            this.showToast('❌ 用户拒绝了通知', '无法发送测试');
                        }
                    });
                }
            }
            ...
            saveSettings() {
                ...
                // 更新铃铛样式
                this.bell.classList.add('subscribed');
                this.bell.innerHTML = '<span class="bell-icon">✅</span><span class="bell-text">提醒已开启</span>';
                
                // 注册定时检查
                this.scheduleChecks(times);
            }
            ...
            sendNotification() {
                // 获取明天的服装信息
                const tomorrowUniform = getTomorrowUniform();
                
                // 发送通知
                if (this.checkPermission()) {
                    const notification = new Notification('学长团提醒', {
                        body: tomorrowUniform,
                        icon: 'https://cdn-icons-png.flaticon.com/512/3135/3135715.png',
                    ...
                    // 显示在网页上也显示一下
                    this.showToast('🔔 新提醒', tomorrowUniform);
                }
            }
            
            showToast(title, detail) {
                this.toastMessage.textContent = title;
                this.toastDetail.textContent = detail;
